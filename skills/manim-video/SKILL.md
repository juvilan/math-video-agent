...
```
① 기획      주제 → 한 문장 핵심 → 콘티        math-storyboard   ← G1
② 씬 분할   비트 → Scene 클래스 (1 Scene = 1 아이디어 = 20~60초)
③ 배치      비트별 **정지 화면만** → sketch 판으로 확인받기       ← G2
③' 코딩     승인된 배치에 애니메이션을 입힌다. 첫 씬 먼저          ← G3
...
애니메이션을 쓰기 전에 **비트별 정지 화면**을 만들어 한 판으로
보여준다. 텍스트 콘티를 승인받은 것과 그림을 승인받은 것은 다르다.

```bash
python3 "$MV" sketch scenes/lorenz.py
```

파일 안의 모든 Scene을 스틸로 렌더해 이름을 각인한 한 장으로
묶는다. **이걸 제작자에게 보여주고 구도·색·밀도를 확인받은 뒤에**
애니메이션을 쓴다. 여기서 "구도가 아니다"가 나오면 버리는 코드가
거의 없다.
...
코드를 안 고치고 조절되는 것들. 수정 요청이 오면 **먼저 이걸로
되는지 본다.**

```bash
MV_PACE=1.25        전체 속도. play(run_time=) 와 wait 에 곱해진다
MV_TEXT_SCALE=1.1   글자 크기
MV_KR_FONT=...      한글 폰트
MV_SFX_DIR=...      효과음 위치
MV_LANG=en          언어
MV_LAYOUT=1         배치 검사 켜기 (mv.py layout 이 자동으로 켠다)
```

프로젝트의 `.mv/intent.md` 에 정해둔 값을 그대로 쓴다.
수정 어휘 사전은 `commands/mv-revise.md`.
